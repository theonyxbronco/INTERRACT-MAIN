(function() {
    ...
    window.rgbKineticSlider = function(options) {
            ...
            if(options.itemsTitles.length > 0) {
                ...
                if( options.textsDisplay == true) {
                    ...
                    for (let i = 0; i < options.itemsTitles.length; i++) {
                        ...
                        // texts centering
                        textTitles.anchor.set(0.62, -0.05); // 0 for x (left) and 0.5 for y (middle)
                        textTitles.x = 1000; // Distance from left edge
        ...
        function cursorInteractive() {
            ...
            let scrollY = 0;
            let headerHeight = window.innerHeight; // Height of your header section
            
            // Original mouse event listeners
            window.addEventListener("mousemove", onPointerMove);
            window.addEventListener("touchmove", onTouchMove);
            
            // Add scroll listener using Locomotive Scroll
            ...
            locoScroll.on('scroll', (args) => {
                ...
                // This will move the effect gradually to the bottom right as you scroll
                if (scrollProgress > 0) {
                    posx = window.innerWidth * (0.5 + (scrollProgress * 0.5));
                    posy = window.innerHeight * (0.5 + (scrollProgress * 0.5));
                    
                    // Increase RGB split effect based on scroll
                    if(options.imagesRgbEffect == true) {
                        const rgbIntensity = options.imagesRgbIntensity * (1 + scrollProgress);
                        splitRgbImgs.red = [rgbIntensity, 0];
                        splitRgbImgs.green = [0, 0];
                        splitRgbImgs.blue = [-rgbIntensity, 0];
                    }
                    
                    // Scale the displacement effect based on scroll
                    dispFilter_2.scale.x = scrollProgress * options.cursorScaleIntensity * 2;
                    dispFilter_2.scale.y = scrollProgress * options.cursorScaleIntensity * 2;
                }
            });
        
            // Original mouse tracking functions
            function onPointerMove(e) {
                // Only update mouse position if we're not scrolling
                if (scrollY <= 0) {
                    posx = e.clientX;
                    posy = e.clientY;
                }
        ...
        function init() {
            ...
            // Update the render loop to handle scroll interactions
            render = new PIXI.Ticker();
            render.autoStart = true;
            render.add(function(delta) {
                renderer.render(stage);
